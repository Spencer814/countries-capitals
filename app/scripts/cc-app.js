...
angular.module('CCApp', ['ngRoute', 'ngAnimate', 'CCLibrary'])
	...
	.controller('capitalCtrl', ['$scope', '$rootScope', '$route', 'countryInfo', 'neighbors', 'timezone', 'capital',
		function($scope, $rootScope, $route, countryInfo, neighbors, timezone, capital){
		...
		timezone($route.current.params.country).then(function(data){
			$scope.timezone = data.geonames;
			$scope.isLoadingCount++;
			if($scope.isLoadingCount == 2) $rootScope.isLoading = false;
		});
